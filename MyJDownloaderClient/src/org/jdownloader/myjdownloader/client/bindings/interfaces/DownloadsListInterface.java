...
public interface DownloadsListInterface extends Linkable {
	...
	/**
	 * Returns the new Counter if the counter does not equal oldCounterValue If
	 * the value changed, we should update the structure. Use this method to
	 * check whether a structure update is required or not
	 * 
	 ...
	 * @param packageIds
	 */
	void setPriority(PriorityStorable priority, long[] linkIds,
			long[] packageIds);
	 ...
	 * 1358496436106,1358496436107],"enabled":true,"size":true,"host":true
	 ...
	 * 
	 *            Optional selectors: packageUUIDs, long[], links contained in
	 *            the packages with given uuids are returned, if empty all links
	 *            are returned startAt, Integer, index of first element to be
	 *            returned maxResults, Integer, total number of elements to be
	 *            returned
	 * 
	...
	void movetoNewPackage(long[] linkIds, long[] pkgIds, String newPkgName,
			String downloadPath);
