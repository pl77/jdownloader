 ...
 * <br>
 * Copyright (c) 2004-2005 Tecnick.com S.r.l (www.tecnick.com) Via Ugo Foscolo n.19 - 09045 Quartu Sant'Elena (CA) - ITALY - www.tecnick.com
 * - info@tecnick.com<br>
...
public class HTMLEntities {
    ...
    private static void initializeEntitiesTables() {
        ...
        for (Object[] element : HTMLEntities.html_entities_table) {
            HTMLEntities.htmlentities_map.put((Integer) element[1], (String) element[0]);
            HTMLEntities.unhtmlentities_map.put((String) element[0], (Integer) element[1]);
        }
    }
        ...
        for (int i = 0; i < str.length(); ++i) {
            ...
            if (ch == '&') {
                ...
                if (semi - i > 7) {
                    ...
                    try {
                        // special handling in case the last entity does not end with ;
                        final String finish = str.substring(i);
                        if (finish.charAt(1) == ' ') {
                            buf.append(ch);
                            continue;
                        }
                        final Integer iso;
                        if (finish.charAt(1) == '#') {
                            if (finish.charAt(2) == 'x') {
                                iso = new Integer(Integer.parseInt(finish.substring(3, finish.length()), 16));
                            } else {
                                iso = new Integer(finish.substring(2, finish.length()));
                            }
                        } else {
                            iso = HTMLEntities.unhtmlentities_map.get(finish);
                        }
                        if (iso != null) {
